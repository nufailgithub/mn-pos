...
type ProductFormValues = UpdateProductInput;
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // Map product sizes to form format
        let sizesData: { size: string; quantity: number }[] = [];
        if (data.freeSize) {
          sizesData = [{ size: "FREE", quantity: data.productSizes[0]?.quantity || 0 }];
        } else {
          // Create a map of existing sizes
          const sizeMap = new Map(
            data.productSizes.map((ps) => [ps.size, ps.quantity])
          );
        ...
        form.reset({
          ...
          brand: data.brand || "",
          stockAlertLimit: data.stockAlertLimit,
  ...
  useEffect(() => {
    if (freeSize) {
      ...
      if (currentSizes.length === 1 && currentSizes[0]?.size === "FREE") {
  ...
  const onSubmit = async (values: ProductFormValues) => {
    try {
      // Generate new barcode on update
      const newBarcode = `BAR-${Date.now()}`;
      ...
      const payload: UpdateProductInput = {
        ...
        barcode: newBarcode, // Always generate new barcode on update
      };
      ...
      // Store updated product and show barcode dialog
      setUpdatedProduct(updated);
      setBarcodeDialogOpen(true);
      
      // Update form with new barcode
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
            ...
            <Form {...form}>
              ...
              >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Barcode Number (New barcode will be generated on update)</FormLabel>
                      <FormControl>
                        <Input readOnly {...field} />
                      </FormControl>
                    </FormItem>
        ...
        {updatedProduct && updatedProduct.barcode && (
          <BarcodeDisplay
